...
void inorder(int node, int** indexes, int* result, int* pos) {
    if (node == -1) return;

    inorder(indexes[node - 1][0], indexes, result, pos);
    result[(*pos)++] = node;
    inorder(indexes[node - 1][1], indexes, result, pos);
}

void swapAtDepth(int node, int depth, int k, int** indexes) {
    if (node == -1) return;
...
int** swapNodes(
    ...
    *result_columns = indexes_rows;
    ...
    for (int i = 0; i < queries_count; i++) {
        ...
        swapAtDepth(1, 1, queries[i], indexes);

        result[i] = malloc(indexes_rows * sizeof(int));
        int pos = 0;
        inorder(1, indexes, result[i], &pos);
    }
